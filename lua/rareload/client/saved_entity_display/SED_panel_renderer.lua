-- 3D2D panel rendering functions
...
function SED.DrawSavedPanel(ent, saved, isNPC)
    ...
    local isDistant = effectiveDistance > (renderParams.isLarge and 800 or 400)
    ...
    local currentLOD = isVeryDistant and 2 or (isDistant and 1 or 0)
    ...
    if isVeryDistant then
        maxVisibleLines = math.min(10, SED.MAX_VISIBLE_LINES)
    elseif isDistant then
        maxVisibleLines = math.min(20, SED.MAX_VISIBLE_LINES)
    end

    local lineHeight = isVeryDistant and 16 or 18
    local titleHeight = isVeryDistant and 28 or 36
    local tabHeight = isVeryDistant and 18 or 22

    local width = cache.widths[activeCat] or 360
    if not cache.widths[activeCat] then
        local font = isVeryDistant and "DermaDefault" or "Trebuchet18"
        ...
        for i = 1, math.min(#lines, 15) do
            ...
            if l and l[1] and l[2] then
                ...
                local lineContentWidth = w1 + w2 + (isVeryDistant and 120 or 170)
                maxContentWidth = math.max(maxContentWidth, lineContentWidth)

                if w2 > (isVeryDistant and 300 or 500) then
                    maxContentWidth = math.min(maxContentWidth, isVeryDistant and 450 or 700)
                end
            end
        end
        ...
        local minTabWidth = isVeryDistant and 40 or 60
        local minWidthForTabs = #categories * minTabWidth
        width = math.max(width, minWidthForTabs)

        local maxWidth = isVeryDistant and 500 or (renderParams.isLarge and 900 or 750)
        local minWidth = isVeryDistant and 250 or 340
    ...
    local worldTop = ent.LocalToWorld and
        ent:LocalToWorld(Vector(obbCenterLocal.x, obbCenterLocal.y, renderParams.obbMax.z)) or
        (pos + Vector(0, 0, renderParams.obbMax.z))
    ...
    local renderOk = pcall(function()
        ...
        if not isVeryDistant then
            SED.surface_SetDrawColor(0, 0, 0, 130)
            surface.DrawRect(offsetX + 4, offsetY + 4, width, panelHeight)
        end

        local bgAlpha = isVeryDistant and 200 or 240
        SED.draw_RoundedBox(isVeryDistant and 6 or 10, offsetX, offsetY, width, panelHeight, Color(15, 18, 26, bgAlpha))
        ...
        local titleFont = isVeryDistant and "Trebuchet18" or "Trebuchet24"
        ...
        local contentFont = isVeryDistant and "DermaDefault" or "Trebuchet18"
        ...
        local labelX = offsetX + (isVeryDistant and 8 or 14)
        local valueX = offsetX + (isVeryDistant and 120 or 180)
        ...
        for i = 1, visibleLines do
            ...
            if not isVeryDistant and (i + currentScroll) % 2 == 0 then
        ...
        if maxScrollLines > 0 and not isVeryDistant then
            local barW = 5
    ...
    local nearestDistSqr, centerWorld = SED.GetNearestDistanceSqr(ent, SED.lpCache:EyePos(), renderParams)
    local detectionPos = centerWorld
    local toEntAng = (detectionPos - SED.lpCache:EyePos()):Angle()
    ...
    local baseRange = math.max(40000,
        math.min(
            renderParams.drawDistanceSqr * (renderParams.isMassive and 0.8 or (renderParams.isLarge and 0.65 or 0.5)),
            500000))
    local yawThreshold = renderParams.isMassive and 32 or (renderParams.isLarge and 24 or 15)
    if not SED.InteractionState.active and nearestDistSqr < baseRange and yawDiff < yawThreshold then
        isCandidate = true
        if not SED.CandidateEnt or yawDiff < (SED.CandidateYawDiff or 999) then
    ...
    if (isFocused or isCandidate) and not isVeryDistant then
