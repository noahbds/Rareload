...
local function CreateModernButton(parent, text, icon, color, onClick)
    ...
    btn.Paint = function(self, w, h)
        ...
        if self:IsHovered() then
            btnColor = Color(math.Clamp(color.r * 1.1, 0, 255), math.Clamp(color.g * 1.1, 0, 255),
                math.Clamp(color.b * 1.1, 0, 255), color.a)
        end
        if self:IsDown() then
            btnColor = Color(math.Clamp(color.r * 0.9, 0, 255), math.Clamp(color.g * 0.9, 0, 255),
                math.Clamp(color.b * 0.9, 0, 255), color.a)
        end
...
function CreateModifyDataButton(actionsPanel, data, isNPC, onAction)
    local modifyBtn = CreateModernButton(actionsPanel, "Modify Data", MAT_PENCIL, THEME.warning, function()
        ...
        editFrame.Paint = function(self, w, h)
            ...
            draw.RoundedBoxEx(12, 0, 0, w, 45, Color(THEME.warning.r, THEME.warning.g, THEME.warning.b, 255), true, true,
                false, false)
        ...
        -- Inject the High-Performance Editor
        if RARELOAD.JSONEditor and RARELOAD.JSONEditor.Create then
            -- Pass the whole entity data; the editor now handles extracting rawData
            RARELOAD.JSONEditor.Create(contentContainer, data, isNPC, function(newData)
                -- =========================================
                -- SAVE LOGIC (Triggered by Ace Editor Save)
                -- =========================================
                            ...
                            if k ~= "__duplicator" then
                                ...
                                if isMatch then
                                    ...
                                    if newData.health then
                                        UpdateField(entityMap[k], "health", "CurHealth",
                                            newData.health)
                                    end
                ...
                if success then
                    ...
                    if writeOk then
                        ...
                        ShowNotification("Failed to write file!", NOTIFY_ERROR)
                    end
                else
            ...
            -- Failsafe if the editor file isn't loaded
