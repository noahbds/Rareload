...
local cleaningUp = false   -- true while Rareload itself runs game.CleanUpMap

-- Removes the player's own objects; with `savedOnly`, only the ones that are part of a save.
...
-- PostPlayerDeath also fires for KillSilent, unlike PlayerDeath (G20, B22).
hook.Add("PostPlayerDeath", "Rareload.Spawn.Died", function(ply)
    ply.rareloadDied = true
end)

-- Must not return a value: the gamemode's PlayerSpawn still has to run and call our
-- PlayerSetModel / PlayerLoadout hooks below.
hook.Add("PlayerSpawn", "Rareload.Spawn.Restore", function(ply, transition)
    ...
    if mode == "all" and not cleaningUp then
        -- Clean the whole map, then spawn the player again; that spawn restores normally (E6, G31).
        cleaningUp = true
...
-- Never called for the singleplayer or listen-server host (G21); see ShutDown.
...
-- An admin cleanup (or any addon calling game.CleanUpMap) would delete everyone's objects, so save
-- each player's world first (E17, F21). Sandbox's gmod_admin_cleanup removes every player's objects
-- before this hook runs, so a module that finds nothing keeps what was saved (keepMissing).
...
-- The host is never "disconnected" (G21). Saving when the pause menu opens was tried and dropped:
-- the console opens it too, so the respawn point moved to wherever the host opened the console.
