...
- **Found in game testing (tool panel)**: the first kit-based tool panel lost v4's look. It is now built like v4's (dark panel, animated categories with an accent bar, switch and slider rows, full-width coloured buttons, footer) plus two additions: a blue dot on settings the player changed from the server's value (right-click to reset) and a lock on locked ones. The spawn menu's stock presets control is dropped; it didn't fit the dark panel.
- **Found in game testing (panels)**: a panel stands on the near side of its own model, the phantom when one shows (v4 anchored to the live object, which left moved objects' phantoms without a panel), at eye height but never below the model's bottom nor above just over its top, and fades out towards the draw distance. A pile's panel stands by its biggest model. Titles are readable names (spawn-menu name, or the model's file name for props) instead of the class.
- **Found in game testing (settings)**: the tool panel lists every server and player setting, like v4's; server settings carry a server icon and only admins can change them. The cleanup, vehicle limit and spawn-limit settings moved to Map Entities, as in v4.
