...
- **Found in game testing (settings)**: each setting shows in one place: player settings in the tool panel, server settings and player defaults in Utilities › Rareload › Server (a lock on each default forces it on every player, replacing the separate locks list), display settings in Utilities › Rareload › Client. The cleanup, vehicle limit and spawn-limit settings moved to Map Entities, as in v4. The tool's right click (save where you stand) no longer shows the tool beam.
